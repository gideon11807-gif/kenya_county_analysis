...
# # ── Page config ──────────────────────────────────────────────────────────────
# st.set_page_config(
#     page_title="Kenya County Budget Analysis",
#     page_icon="🇰🇪",
#     layout="wide"
# )
...
#         <div class="metric-value metric-accent">{df['population_2024'].sum()/1e6:.1f}M</div>
...
#         <div class="metric-value">KES {df['budget_billion'].sum():.1f}B</div>
#     </div>""", unsafe_allow_html=True)

# with c3:
#     st.markdown(f"""
#     <div class="metric-card">
#         <div class="metric-label">National Average / Person</div>
...
#     st.markdown(f"""
#     <div class="metric-card">
#         <div class="metric-label">Highest vs Lowest Gap</div>
#         <div class="metric-value metric-accent-red">KES {gap:,}</div>
#     </div>""", unsafe_allow_html=True)

# # ── Chart 1: All 47 Counties ─────────────────────────────────────────────────
...
# ax1.bar(sorted_df['county'], sorted_df['budget_per_person'], color=colors, edgecolor='#0a0f1e', linewidth=0.4, width=0.75)
# ax1.axhline(df['budget_per_person'].mean(), color='#f59e0b', linestyle='--', linewidth=1.5,
#             label=f"National Average: KES {int(df['budget_per_person'].mean()):,}")
...
# plt.xticks(rotation=90, fontsize=7.5, color=MUTED)
# plt.tight_layout()
# st.pyplot(fig1)

# # ── Chart 2: Top 10 / Bottom 10 ──────────────────────────────────────────────
# st.markdown('<div class="section-header">Top 10 vs Bottom 10 Counties</div>', unsafe_allow_html=True)
...
#     bars = ax2.barh(top10['county'], top10['budget_per_person'], color=GREEN, edgecolor='#0a0f1e', linewidth=0.4)
#     ax2.set_title('Highest Budget Per Person', fontsize=11, fontweight='bold', color=TEXT)
#     ax2.xaxis.set_major_formatter(mticker.FuncFormatter(lambda x, _: f'{int(x):,}'))
#     ax2.set_xlabel('KES per Person', fontsize=9)
#     ax2.invert_yaxis()
...
#     ax3.barh(bot10['county'], bot10['budget_per_person'], color=RED, edgecolor='#0a0f1e', linewidth=0.4)
#     ax3.set_title('Lowest Budget Per Person', fontsize=11, fontweight='bold', color=TEXT)
#     ax3.xaxis.set_major_formatter(mticker.FuncFormatter(lambda x, _: f'{int(x):,}'))
#     ax3.set_xlabel('KES per Person', fontsize=9)
#     ax3.invert_yaxis()
#     ax3.tick_params(axis='y', labelsize=9, colors=TEXT)
#     plt.tight_layout()
#     st.pyplot(fig3)

# # ── Chart 3: Scatter ──────────────────────────────────────────────────────────
# st.markdown('<div class="section-header">Budget vs Population</div>', unsafe_allow_html=True)
...
#     cmap='RdYlGn',
#     s=100, alpha=0.9, edgecolors='#1e2d4a', linewidth=0.8
# )

# for _, row in df.iterrows():
#     if row['county'] in ['Nairobi', 'Lamu', 'Turkana', 'Nakuru', 'Kiambu', 'Kakamega', 'Mombasa', 'Kisumu']:
#         ax4.annotate(row['county'],
#                      (row['population_2024'] / 1_000_000, row['budget_billion']),
#                      textcoords='offset points', xytext=(7, 4),
#                      fontsize=8.5, color=TEXT,
#                      arrowprops=dict(arrowstyle='-', color=MUTED, lw=0.6))

# cb = plt.colorbar(scatter, ax=ax4)
# cb.set_label('Budget Per Person (KES)', color=MUTED, fontsize=9)
# cb.ax.yaxis.set_tick_params(color=MUTED, labelcolor=MUTED)
# ax4.set_xlabel('Population (millions)', fontsize=10)
# ax4.set_ylabel('Budget (KES Billions)', fontsize=10)
# plt.tight_layout()
# st.pyplot(fig4)
...
#             <div class="metric-value" style="font-size:1.4rem">{int(row['population_2024']):,}</div>
#         </div>""", unsafe_allow_html=True)
#     with cb2:
#         st.markdown(f"""<div class="metric-card">
#             <div class="metric-label">Budget</div>
#             <div class="metric-value" style="font-size:1.4rem">KES {row['budget_billion']}B</div>
#         </div>""", unsafe_allow_html=True)
#     with cc:
#         st.markdown(f"""<div class="metric-card">
#             <div class="metric-label">Per Person · Rank #{rank}</div>
#             <div class="metric-value {diff_color}" style="font-size:1.4rem">KES {int(row['budget_per_person']):,}</div>
#         </div>""", unsafe_allow_html=True)

#     st.caption(f"**{selected}** is **{diff_str}** vs the national average of KES {nat_avg:,}/person — ranked **#{rank} of 47**.")

# # ── Full Table ────────────────────────────────────────────────────────────────
# st.markdown('<div class="section-header">Full County Rankings</div>', unsafe_allow_html=True)

# table = sorted_df[['county', 'population_2024', 'budget_billion', 'budget_per_person']].copy()
# table.columns = ['County', 'Population (2024)', 'Budget (KES B)', 'Budget/Person (KES)']
